...
async function publishPoetry(pluginConfig, logger) {
  ...
  if (pypiPublish) {
    try {
      logger.log(`Configure Poetry repository ${repoUrl} for deployment`)
      await execa('poetry', ['config', 'repositories.repo.url', repoUrl], { cwd: path.dirname(setupPy) })
    } catch {
      throw new Error(`Failed to run "poetry config repositories.repo ${repoUrl}"`)
    }
    if (repoCert) {
      try {
        logger.log(`Configure Poetry repository ${repoUrl} to use '${repoCert}'`)
        await execa('poetry', ['config', 'certificates.repo.cert', repoCert], { cwd: path.dirname(setupPy) })
      } catch (error) {
        throw new Error(error)
      }
    }
    try {
      logger.log(`Publishig to ${repoUrl} with poetry`)
      await execa(
        'poetry',
        [
          'publish',
